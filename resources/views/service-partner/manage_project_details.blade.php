...
@section('content')
  ...
  <div class="container">
    ...
    <div class="mb-4">
    <h2 class="fw-bold">Project Milestones</h2>
    </div>
    </br>
    ...
    <table class="table table-hover" id="myTable">
      ...
      @foreach ($projectPlanner as $pplnr)

      <tr>
      <td>{{ $pplnr->pplnr_milestone ?: 'No milestone available' }}</td>
      <td>{{ $pplnr->pplnr_description ?: 'No description available' }}</td>
      <td>{{ $pplnr->pplnr_start_date ?: 'No start date available' }}</td>
      <td>{{ $pplnr->pplnr_end_date ?: 'No end date available' }}</td>
      <td>{{ $pplnr->pplnr_status ?: 'No status available' }}</td>
      <td>
      <a href="{{ url('service-partner/session/manage_project_view_tasks/' . $pplnr->pplnr_id)}}"
