  ...
  <div class="container">
    ...
    <table class="table table-hover" id="myTable">
      ...
      <tbody>
        ...
        @if ($hardwareOrders->isEmpty())
          <tr>
            <td colspan="6" class="text-center">No orders found.</td>
          </tr>

        @elseif ($hardwareOrders->isNotEmpty())
          @foreach ($hardwareOrders as $order)
            <tr>
              <th scope="row">{{ $order->ordplcd_order_no }}</th>
              <td>{{ $order->ordplcd_qty_placed }}</td>
              <td>{{ $order->ordplcd_order_date }}</td>
              <td>{{ $order->ordplcd_status }}</td>
              <td>{{ $order->ordplcd_amt }}</td>
              <td><a href="{{ url('/service-partner/session/hardware-details-sp/' . $order->ordplcd_hw_id . '/' . $order->ordplcd_id) }}" class="btn btn-sm btn-outline-primary"
